'use strict';
const async = require('async');
const semver = require('semver');
const rp =require('request-promise');
const Promise =require('bluebird')
const settings=require('../settings')

module.exports = function(agenda, options) {
  options = options || {};

  agenda.on('ready', () => {
    const collection = agenda._collection.collection || agenda._collection;
    collection.createIndexes([
      {key: {nextRunAt: -1, lastRunAt: -1, lastFinishedAt: -1}},
      {key: {name: 1, nextRunAt: -1, lastRunAt: -1, lastFinishedAt: -1}}
    ], err => {
      if (err) {
        console.warn('Agendash indexes might not exist. Performance may decrease.');
      }
    });
    agenda._mdb.admin().serverInfo((err, serverInfo) => {
      if (err) {
        throw err;
      }
      if (!semver.satisfies(serverInfo.version, '>=2.6.0')) {
        console.warn('Agendash requires mongodb version >=2.6.0.');
      }
    });
  });

  const getJobs = (job, state, callback) => {
    const preMatch = {};
    if (job) {
      preMatch.name = job;
    }

    const postMatch = {};
    if (state) {
      postMatch[state] = true;
    }

    const limit = 200; // @TODO: UI param
    const skip = 0; // @TODO: UI param

    const collection = agenda._collection.collection || agenda._collection;
    collection.aggregate([
      {$match: preMatch},
      {$sort: {
        nextRunAt: -1,
        lastRunAt: -1,
        lastFinishedAt: -1
      }},
      {$project: {
        _id: 0,
        job: '$$ROOT',
        nextRunAt: {$ifNull: ['$nextRunAt', 0]},
        lockedAt: {$ifNull: ['$lockedAt', 0]},
        lastRunAt: {$ifNull: ['$lastRunAt', 0]},
        lastFinishedAt: {$ifNull: ['$lastFinishedAt', 0]},
        failedAt: {$ifNull: ['$failedAt', 0]},
        repeatInterval: {$ifNull: ['$repeatInterval', 0]}
      }},
      {$project: {
        job: '$job',
        _id: '$job._id',
        running: {$and: [
          '$lastRunAt',
          {$gt: ['$lastRunAt', '$lastFinishedAt']}
        ]},
        scheduled: {$and: [
          '$nextRunAt',
          {$gte: ['$nextRunAt', new Date()]}
        ]},
        queued: {$and: [
          '$nextRunAt',
          {$gte: [new Date(), '$nextRunAt']},
          {$gte: ['$nextRunAt', '$lastFinishedAt']}
        ]},
        completed: {$and: [
          '$lastFinishedAt',
          {$gt: ['$lastFinishedAt', '$failedAt']}
        ]},
        failed: {$and: [
          '$lastFinishedAt',
          '$failedAt',
          {$eq: ['$lastFinishedAt', '$failedAt']}
        ]},
        repeating: {$and: [
          '$repeatInterval',
          {$ne: ['$repeatInterval', null]}
        ]}
      }},
      {$match: postMatch},
      {$limit: limit},
      {$skip: skip}
    ]).toArray((err, results) => {
      if (err) {
        return callback(err);
      }
      callback(null, results);
    });
  };

  const getOverview = callback => {
    const collection = agenda._collection.collection || agenda._collection;
    collection.aggregate([
      {$project: {
        _id: 0,
        name: '$name',
        type: '$type',
        priority: '$priority',
        repeatInterval: '$repeatInterval',
        repeatTimezone: '$repeatTimezone',
        nextRunAt: {$ifNull: ['$nextRunAt', 0]},
        lockedAt: {$ifNull: ['$lockedAt', 0]},
        lastRunAt: {$ifNull: ['$lastRunAt', 0]},
        lastFinishedAt: {$ifNull: ['$lastFinishedAt', 0]},
        failedAt: {$ifNull: ['$failedAt', 0]}
      }},
      {$project: {
        name: '$name',
        type: '$type',
        priority: '$priority',
        repeatInterval: '$repeatInterval',
        repeatTimezone: '$repeatTimezone',
        running: {$cond: [{$and: [
          '$lastRunAt',
          {$gt: ['$lastRunAt', '$lastFinishedAt']}
        ]}, 1, 0]},
        scheduled: {$cond: [{$and: [
          '$nextRunAt',
          {$gte: ['$nextRunAt', new Date()]}
        ]}, 1, 0]},
        queued: {$cond: [{$and: [
          '$nextRunAt',
          {$gte: [new Date(), '$nextRunAt']},
          {$gte: ['$nextRunAt', '$lastFinishedAt']}
        ]}, 1, 0]},
        completed: {$cond: [{$and: [
          '$lastFinishedAt',
          {$gt: ['$lastFinishedAt', '$failedAt']}
        ]}, 1, 0]},
        failed: {$cond: [{$and: [
          '$lastFinishedAt',
          '$failedAt',
          {$eq: ['$lastFinishedAt', '$failedAt']}
        ]}, 1, 0]},
        repeating: {$cond: [{$and: [
          '$repeatInterval',
          {$ne: ['$repeatInterval', null]}
        ]}, 1, 0]}
      }},
      {$group: {
        _id: '$name',
        displayName: {$first: '$name'},
        meta: {$addToSet: {
          type: '$type',
          priority: '$priority',
          repeatInterval: '$repeatInterval',
          repeatTimezone: '$repeatTimezone'
        }},
        total: {$sum: 1},
        running: {$sum: '$running'},
        scheduled: {$sum: '$scheduled'},
        queued: {$sum: '$queued'},
        completed: {$sum: '$completed'},
        failed: {$sum: '$failed'},
        repeating: {$sum: '$repeating'}
      }}
    ]).toArray((err, results) => {
      if (err) {
        return callback(err);
      }
      const totals = {displayName: 'All Jobs'};
      const states = ['total', 'running', 'scheduled', 'queued', 'completed', 'failed', 'repeating'];
      states.forEach(state => {
        totals[state] = 0;
      });
      results.forEach(job => {
        states.forEach(state => {
          totals[state] += job[state];
        });
      });
      results.unshift(totals);
      callback(null, results);
    });
  };

  const api = function(job, state, callback) {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    async.parallel({
      overview: getOverview,
      jobs: getJobs.bind(this, job, state)
    },
    (err, apiResponse) => {
      if (err) {
        return callback(err.message);
      }
      apiResponse.title = options.title || 'Agendash';
      apiResponse.currentRequest = {
        title: options.title || 'Agendash',
        job: job || 'All Jobs',
        state
      };
      callback(null, apiResponse);
    });
  };

  const requeueJobs = (jobIds, callback) => {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    try {
      const collection = agenda._collection.collection || agenda._collection;
      collection
      .find({_id: {$in: jobIds.map(jobId => collection.s.pkFactory(jobId))}})
      .toArray((err, jobs) => {
        if (err || jobs.length === 0) {
          return callback('Jobs not found');
        }
        async.series(jobs.map(job => done => {
          const newJob = agenda.create(job.name, job.data).save(() => {
            done(null, newJob);
          });
        }), (err, results) => {
          callback(err, results);
        });
      });
    } catch (err) {
      callback(err.message);
    }
  };

  const deleteJobs = (jobIds, callback) => {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    try {
      const collection = agenda._collection.collection || agenda._collection;
      agenda.cancel({_id: {$in: jobIds.map(jobId => collection.s.pkFactory(jobId))}}, (err, deleted) => {
        if (err || !deleted) {
          callback('Jobs not deleted');
        }
        callback();
      });
    } catch (err) {
      callback(err.message);
    }
  };

  const createJob = (jobName, jobSchedule, jobRepeatEvery, jobData, callback) => {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    try {
      // @TODO: Need to validate user input.
        const job = agenda.create(jobName, jobData);
        let scheduleDate=jobSchedule
        let timestamp=new Date(scheduleDate).getTime()
        if(timestamp){
            timestamp=timestamp-1000*60*60*12
            scheduleDate=new Date(timestamp)
            let year=scheduleDate.getFullYear()
            let month=scheduleDate.getMonth()+1-10 >= 0 ? (scheduleDate.getMonth()+1) : `0${scheduleDate.getMonth()+1}`
            let day=scheduleDate.getDate() -10 >= 0 ? (scheduleDate.getDate()) : `0${scheduleDate.getDate()}`
            let hour=scheduleDate.getHours() -10 >= 0 ? (scheduleDate.getHours()) : `0${scheduleDate.getHours()}`
            let minute=scheduleDate.getMinutes() -10 >= 0 ? (scheduleDate.getMinutes()) : `0${scheduleDate.getMinutes()}`
            let seconds=scheduleDate.getSeconds() -10 >= 0 ? (scheduleDate.getSeconds()) : `0${scheduleDate.getSeconds()}`
            scheduleDate=`${year}-${month}-${day} ${hour}:${minute}:${seconds}`
        }
      if (scheduleDate && jobRepeatEvery) {
        job.repeatAt(scheduleDate);
        job.repeatEvery(jobRepeatEvery,{
            timezone: 'Asia/Shanghai'
        });
      } else if (scheduleDate) {
        job.schedule(scheduleDate);
      } else if (jobRepeatEvery) {
        job.repeatEvery(jobRepeatEvery,{
            timezone: 'Asia/Shanghai'
        });
      } else {
        return callback('Jobs not created');
      }
      job.save(err => {
        if (err) {
          return callback('Jobs not created');
        }
        callback();
      });
    } catch (err) {
      callback(err.message);
    }
  };

  //定义job

  const definitions = function(definition, state, callback) {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    try {
      Promise.race([
        new Promise((resolve, reject) => setTimeout(() => reject(new Error('TimeOutError')), settings.timeout)),
        rp({
          method: 'GET',
          uri:settings.agenda_rest_url,
          body: {
          },
          json: true
        })
      ])
        .catch(err => {
          return callback(err.message);
        })
        .then(result => {
          var responseData={}
          responseData.definitions=result.map((de)=>{
                  var data={}
                  data.definition=de
                  return data
          })
          responseData.title = options.title || 'Agendash';
          responseData.currentRequest = {
            title: options.title || 'Agendash',
            definition: definition || 'All Definitions',
            state
          };
          callback(null, responseData);
        })
    }catch (err) {
            return callback(err.message);
        }


  };



  const deleteDefinitions = (definitionNames, callback) => {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    try {
      Promise.race([
        new Promise((resolve, reject) => setTimeout(() => reject(new Error('TimeOutError')), settings.timeout)),
        rp({
          method: 'DELETE',
          uri:`${settings.agenda_rest_url}/${definitionNames[0]}`,
          body: {
          },
          json: true
        })
      ]).then(result=>{
          return callback()
      })
        .catch(err => {
          return callback(err.message);
        })
    } catch (err) {
      return callback(err.message);
    }
  };

  const defineDefinition = (name, url, method,callbackData ,callback) => {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    Promise.race([
      new Promise((resolve, reject) => setTimeout(() => reject(new Error('TimeOutError')), settings.timeout)),
      rp({
        method: method || 'POST',
        uri:settings.agenda_rest_url,
        body: {
          name,
          url,
          method,
          callback:callbackData
        },

        json: true
      })
    ])
      .catch(err => {
        return callback(err.message);
      })
      .then(result => {
        return callback();
      })
  };

  return {
    api,
    requeueJobs,
    deleteJobs,
    createJob,
    definitions,
   // updateDefinitions,
    deleteDefinitions,
    defineDefinition
  };
};
